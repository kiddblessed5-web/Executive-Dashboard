  ...
  return [
    {
      ...
      membersLabel: "24 Members, 6 Online",
      convPinned: true,
      description:
        "Daily coordination for the Vivo production line — unboxing through packaging.",
    },
    {
      id: "c-qc",
      type: "channel",
      name: "quality-control",
      membersLabel: "9 Members, 3 Online",
      convPinned: false,
      description:
        "Quality inspection queue, defect reports and reseal approvals.",
    },
    {
      id: "c-announce",
      type: "channel",
      name: "announcements",
      membersLabel: "42 Members, 11 Online",
      convPinned: true,
    ...
    },
    {
      id: "g-managers",
      type: "group",
      name: "Chinese Managers",
      membersLabel: "4 Members, 2 Online",
      convPinned: false,
      description:
        "Coordination group for Wei, Li, Feng and Chao on batch assignment.",
    },
    ...
    "c-floor": [
      {
        id: "m1",
        sender: "wei",
        text: "Morning team — vote for the meeting guys, when\u2019s the best time?",
        time: t(240),
        reactions: {},
        pinned: true,
        poll: {
          question: "Best time for today\u2019s sync?",
          options: [
            { label: "10:00 AM", votes: 6 },
            { label: "12:00 PM", votes: 3 },
            { label: "2:00 PM", votes: 1 },
          ],
          myVote: null,
        },
      },
      {
        id: "m2",
        sender: "grace",
        text: "Unboxing is done, moving to software install now.",
        time: t(210),
        reactions: { "👍": 2 },
        pinned: false,
      },
      {
        id: "m3",
        sender: "kevin",
        text: "QC queue is a bit backed up, will prioritise BX-1042.",
        time: t(180),
        reactions: {},
        pinned: true,
      },
      {
        id: "m4",
        sender: "me",
        text: "Thanks Kevin, appreciate it 🙏",
        time: t(170),
        reactions: {},
        pinned: false,
      },
      {
        id: "m5",
        sender: "brian",
        text: "Delivered 300 more Vivo Y18 units to the warehouse, ref BX-1051.",
        time: t(90),
        reactions: { "🎉": 1 },
        pinned: false,
        attachment: {
          type: "file",
          name: "delivery-note-BX-1051.pdf",
          size: "214 KB",
        },
      },
      {
        id: "m6",
        sender: "me",
        text: "Wait a minute, I\u2019ll check my schedule first",
        time: t(60),
        reactions: {},
        pinned: false,
      },
    ...
    "c-qc": [
      {
        id: "m1",
        sender: "kevin",
        text: "Failed 4 units from BX-1039 — screen defects. Photos attached.",
        time: t(300),
        reactions: {},
        pinned: false,
      ...
      },
      {
        id: "m2",
        sender: "wei",
        text: "Noted, please reseal the rest and flag the 4 for return.",
        time: t(280),
        reactions: { "👍": 1 },
        pinned: false,
      },
    ...
    "c-announce": [
      {
        id: "m1",
        sender: "hr",
        text: "Reminder: Warehouse B will run extended hours this Saturday for the Vivo Y-series batch. Overtime pay applies.",
        time: t(600),
        reactions: { "👍": 6, "🎉": 2 },
        pinned: true,
      },
    ...
    "g-managers": [
      {
        id: "m1",
        sender: "li",
        text: "Can we sync on next week\u2019s production targets?",
        time: t(400),
        reactions: {},
        pinned: false,
      },
      {
        id: "m2",
        sender: "wei",
        text: "Yes — let\u2019s do Monday 9am before the floor opens.",
        time: t(390),
        reactions: {},
        pinned: false,
      },
    ...
    "dm-wei": [
      {
        id: "m1",
        sender: "wei",
        text: "Hey Alex, do you have the worker allocation for tomorrow\u2019s shift?",
        time: t(50),
        reactions: {},
        pinned: false,
      },
      {
        id: "m2",
        sender: "me",
        text: "Sending it over now, one sec.",
        time: t(45),
        reactions: {},
        pinned: false,
      },
    ...
    "dm-grace": [
      {
        id: "m1",
        sender: "grace",
        text: "Hit 312 units today \u2014 new personal best 🚀",
        time: t(70),
        reactions: { "🔥": 1 },
        pinned: false,
      },
    ...
    "dm-kevin": [
      {
        id: "m1",
        sender: "kevin",
        text: "Voice note attached re: the QC backlog",
        time: t(120),
        reactions: {},
        pinned: false,
        attachment: { type: "voice", duration: "0:34" },
      },
    ],
    "dm-brian": [
      {
        id: "m1",
        sender: "brian",
        text: "Next delivery is scheduled for Thursday morning.",
        time: t(500),
        reactions: {},
        pinned: false,
      },
    ...
    "dm-hr": [
      {
        id: "m1",
        sender: "hr",
        text: "Your payroll for this week has been processed ✅",
        time: t(200),
        reactions: {},
        pinned: false,
      },
...
function loadMessagesState() {
  ...
  unreadState = savedUnread
    ? JSON.parse(savedUnread)
    : { "c-qc": 2, "dm-kevin": 1, "g-managers": 1 };
}
...
function persistUnread() {
  localStorage.setItem("nexus_unread", JSON.stringify(unreadState));
}
...
function renderOnlineNow() {
  ...
  const online = Object.entries(MSG_PEOPLE).filter(
    ([id, p]) => id !== "me" && p.online,
  );
...
async function openDMWith(personId) {
  let conv = CONVERSATIONS.find(
    (c) => c.type === "dm" && c.personId === personId,
  );
...
async function createDMConversation(personId) {
  if (SagoBackend?.isConfigured()) {
    ...
    const { data: newConv, error } = await sb
      .from("conversations")
      .insert({ type: "dm", created_by: backendUserId })
      .select()
      .single();
    if (error) {
      NexusApp.toast("Could not start conversation: " + error.message, "error");
      return null;
    }
...
function renderComposeList() {
  ...
  const filtered = people.filter(
    ([id, p]) =>
      !composeSearch ||
      p.name.toLowerCase().includes(composeSearch) ||
      p.role.toLowerCase().includes(composeSearch),
  );
  list.innerHTML =
    filtered
      .map(([id, p]) => {
        const hasConv = CONVERSATIONS.some(
          (c) => c.type === "dm" && c.personId === id,
        );
        return `
    <div class="compose-opt" onclick="startNewConversation('${id}')">
      ...
      .join("") ||
    `<div class="muted-note" style="padding:16px;">No one matches your search.</div>`;
}
async function startNewConversation(personId) {
  ...
  if (isNew)
    NexusApp.toast(
      `Started a conversation with ${MSG_PEOPLE[personId].name}`,
      "success",
    );
}
...
function convRowHTML(c) {
  ...
  const lastText = last
    ? last.text || attachmentPreviewText(last)
    : "No messages yet";
...
function renderConvList() {
  ...
  const all = CONVERSATIONS.filter(
    (c) => !q || convDisplayName(c).toLowerCase().includes(q),
  );
  const pinned = all.filter((c) => c.convPinned);
  const rest = all.filter((c) => !c.convPinned);

  document.getElementById("pinnedSection").style.display = pinned.length
    ? "block"
    : "none";
  document.getElementById("pinnedList").innerHTML = pinned
    .map(convRowHTML)
    .join("");
  document.getElementById("allList").innerHTML =
    rest.map(convRowHTML).join("") ||
    `<div class="muted-note" style="padding:16px;">No conversations match.</div>`;
}
...
function renderActiveHeader() {
  ...
  document.getElementById("activeSub").textContent = isDM
    ? MSG_PEOPLE[c.personId].online
      ? "Online"
      : "Offline"
    : c.membersLabel;
  document.getElementById("activeStatusDot").style.display = isDM
    ? "inline-block"
    : "none";
  document.getElementById("activeStatusDot").className =
    "status-dot-inline " +
    (isDM && MSG_PEOPLE[c.personId].online ? "online" : "offline");
}
      ...
      <div class="msg-bubble-col">
        ...
        ${
          Object.keys(m.reactions || {}).length
            ? `<div class="msg-reactions">${Object.entries(m.reactions)
                .map(
                  ([emoji, count]) =>
                    `<span class="reaction-chip" onclick="toggleReaction('${m.id}','${emoji}')">${emoji} ${count}</span>`,
                )
                .join("")}</div>`
            : ""
        }
...
function renderPinnedBanner() {
  ...
  document.getElementById("pinnedText").textContent =
    `${MSG_PEOPLE[first.sender].name}: ${first.text || attachmentPreviewText(first)}`;
  document.getElementById("pinnedCount").textContent =
    pinned.length > 1 ? `+${pinned.length - 1} more` : "";
}

function renderMentions(text) {
  return text.replace(/@([A-Za-z]+)/g, (match, name) => {
    const found = MENTIONABLE.find((p) =>
      p.name.toLowerCase().startsWith(name.toLowerCase()),
    );
    return found
      ? `<span class="mention-chip">@${found.name.split(" ")[0]}</span>`
      : match;
  });
}
function renderAttachment(att) {
  ...
  if (att.type === "voice") {
    return `<div class="att-voice" onclick="this.classList.toggle('playing')"><i class="ri-play-fill att-voice-icon"></i><div class="att-voice-wave">${Array.from(
      { length: 18 },
    )
      .map(
        () =>
          `<span style="height:${6 + Math.round(Math.random() * 16)}px"></span>`,
      )
      .join("")}</div><span class="att-voice-dur">${att.duration}</span></div>`;
  }
  return "";
}
...
function toggleReaction(msgId, emoji) {
  if (SagoBackend?.isConfigured()) {
    ...
    sb.from("message_reactions")
      .insert({ message_id: msgId, user_id: backendUserId, emoji })
      .then(({ error }) => {
        if (error && error.code !== "23505") {
          NexusApp.toast("Reaction failed: " + error.message, "error");
          return;
        }
        const msgs = MESSAGES[activeConvId];
        const m = msgs?.find((x) => x.id === msgId);
...
function openReactPicker(evt, msgId) {
  ...
  pop.innerHTML = QUICK_EMOJIS.map(
    (e) =>
      `<span onclick="toggleReaction('${msgId}','${e}'); closeAllPopovers();">${e}</span>`,
  ).join("");
...
function togglePin(msgId) {
  ...
  if (SagoBackend?.isConfigured()) {
    ...
    sb.from("messages")
      ...
      .then(({ error }) => {
        if (error) {
          NexusApp.toast("Could not update pin: " + error.message, "error");
          return;
        }
        m.pinned = nextPinned;
        renderMessages();
        NexusApp.toast(
          m.pinned ? "Message pinned" : "Message unpinned",
          "info",
        );
      });
    return;
...
document.addEventListener("click", (e) => {
  if (
    ...
    !e.target.closest("#reactPopover")
  )
    document.getElementById("reactPopover")?.classList.remove("open");
  if (!e.target.closest("#emojiBtn") && !e.target.closest("#emojiPopover"))
    document.getElementById("emojiPopover")?.classList.remove("open");
});

/* ---------------- SEND / TYPING / ATTACH / MENTIONS ---------------- */
function genId() {
  return "m" + Date.now() + Math.floor(Math.random() * 999);
}
...
function attachMock(type) {
  ...
  if (type === "image")
    attachment = {
      type: "image",
      label: "photo-" + Math.floor(Math.random() * 900) + ".jpg",
    };
  if (type === "file")
    attachment = { type: "file", name: "shift-report.pdf", size: "188 KB" };
  if (type === "voice")
    attachment = {
      type: "voice",
      duration: "0:" + (10 + Math.floor(Math.random() * 40)),
    };
  MESSAGES[activeConvId].push({
    id: genId(),
    sender: "me",
  ...
  renderConvList();
  NexusApp.toast(
    type[0].toUpperCase() + type.slice(1) + " attached",
    "success",
  );
}
...
function toggleEmojiPicker() {
  ...
  pop.innerHTML = QUICK_EMOJIS.map(
    (e) => `<span onclick="insertEmoji('${e}')">${e}</span>`,
  ).join("");
  ...
  if (caretAtMention) {
    ...
    if (matches.length) {
      pop.innerHTML = matches
        .map(
          (p) =>
            `<div class="mention-opt" onclick="pickMention('${p.name.replace(/'/g, "")}')"><span>${initials(p.name)}</span>${p.name}</div>`,
        )
        .join("");
      pop.classList.add("open");
...
function renderInfoPanel() {
  ...
  document.getElementById("infoAvatar").innerHTML = isDM
    ? initials(convDisplayName(c))
    : `<i class="${c.type === "channel" ? "ri-hashtag" : "ri-group-line"}"></i>`;
  ...
  document.getElementById("infoSub").textContent = isDM
    ? MSG_PEOPLE[c.personId].online
      ? "Online"
      : "Offline"
    : c.membersLabel;
  ...
  const msgs = MESSAGES[activeConvId] || [];
  const mediaCount =
    msgs.filter((m) => m.attachment && m.attachment.type === "image").length +
    12;
  document.getElementById("infoMediaCount").textContent = mediaCount;

  const files = seedFiles();
  document.getElementById("infoFilesList").innerHTML = files
    .map(
      (f) => `
...
function toggleInfoPanel() {
  ...
  document
    .getElementById("msgShell")
    .classList.toggle("info-hidden", !infoPanelOpen);
}

/* ---------------- SEARCH ---------------- */
function wireSidebarControls() {
  document.getElementById("convSearch").addEventListener("input", (e) => {
    msgSearch = e.target.value.trim().toLowerCase();
    renderConvList();
  });
  document.getElementById("composeSearch").addEventListener("input", (e) => {
    composeSearch = e.target.value.trim().toLowerCase();
    renderComposeList();
  });
}
...
async function initBackendMessaging(session) {
  ...
  if (profileErr) {
    NexusApp.toast(
      "Could not load team directory: " + profileErr.message,
      "error",
    );
  }
  MSG_PEOPLE["me"] = {
  ...
  const { data: memberRows, error: memberErr } = await sb
    ...
    .select(
      "conversation_id, conversations(id, type, name, created_by), user_id",
    )
    .eq("user_id", backendUserId);
  if (memberErr) {
    NexusApp.toast(
      "Could not load conversations: " + memberErr.message,
      "error",
    );
    return;
  }
  ...
  CONVERSATIONS = (memberRows || []).map((r) => {
    ...
    const otherMemberId = allMembers.find(
      (m) => m.conversation_id === conv.id && m.user_id !== backendUserId,
    )?.user_id;
    return {
      id: conv.id,
      type: conv.type,
      ...
      membersLabel:
        conv.type !== "dm"
          ? `${allMembers.filter((m) => m.conversation_id === conv.id).length} Members`
          : undefined,
      convPinned: false,
      description: "",
    };
...
async function loadBackendMessages(conversationId) {
  ...
  if (error) {
    NexusApp.toast("Could not load messages: " + error.message, "error");
    return;
  }
  ...
  MESSAGES[conversationId] = (data || []).map((m) => ({
    ...
    pinned: m.pinned,
    reactions: (m.message_reactions || []).reduce((acc, r) => {
    ...
    .on(
      ...
      async (payload) => {
        ...
        if (m.conversation_id === activeConvId) {
          ...
          unreadState[m.conversation_id] =
            (unreadState[m.conversation_id] || 0) + 1;
        }
        renderConvList();
      },
    )
...
async function sendMessageBackend(text) {
  ...
  if (error)
    NexusApp.toast("Message failed to send: " + error.message, "error");
